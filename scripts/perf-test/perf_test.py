...
def get_default_vpc(session, region, az):
    ...
    subnet = res['Subnets'][0]['SubnetId']
    logger.info(region + ': default subnet in ' + az + ': ' + subnet)
...
def prepare_ebs(session, region, az, subnet, ami):
...
def build(session, region, az, subnet, ami, repository, commit_id, bucket_name,
...
def run_perf_test(
        session, mode, region, az, vpc, subnet, ami, instance_type,
        sqs_arn, sqs_url,
...
def main():
    ...
    run_perf_test(
            session, args.mode, region, az, vpc, subnet, ami,
            args.instance_type,
