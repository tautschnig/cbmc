/// \file
/// test_unit.c — unit tests for the kref_lifetime property
/// module.
...
// Concrete `struct kref` for this abstract test.  The
...
int main(void)
{
  ...
  kref_lifetime_init(&b, 1);
  __CPROVER_assert(kref_live(&b) == 1, "usage=1 is live");
  kref_lifetime_put(&b);
  __CPROVER_assert(
    kref_live(&b) == 0, "after single put, not live");

  // test 3: init with usage=2 -> live; get -> live; put thrice
  //         brings usage from 3 down to 0.
  kref_lifetime_init(&c, 2);
  kref_lifetime_get(&c);
  __CPROVER_assert(kref_live(&c) == 1, "usage=3 still live");
  kref_lifetime_put(&c);
  __CPROVER_assert(kref_live(&c) == 1, "usage=2 still live");
  kref_lifetime_put(&c);
  __CPROVER_assert(kref_live(&c) == 1, "usage=1 still live");
  kref_lifetime_put(&c);
  __CPROVER_assert(kref_live(&c) == 0, "usage=0 is dead");

  // test 4: NULL -> not live (defensive).
  __CPROVER_assert(
    kref_live((struct kref *)0) == 0,
    "NULL is not live");
