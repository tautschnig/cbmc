#!/usr/bin/env bash
#
# Regression driver for the refcount_lifetime property module.
#
# One test: unit exercise of the ghost store + refcount_live
...
echo "=== unit: refcount_lifetime ghost + refcount_live predicate ==="
run_gotocc refcount_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "refcount_lifetime tests behaved as expected."
  exit 0
fi
