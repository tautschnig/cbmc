#!/usr/bin/env bash
#
# Regression driver for the inode_lifetime property module.
#
# One test: unit exercise of the ghost store + inode_live
...
echo "=== unit: inode_lifetime ghost + inode_live predicate ==="
run_gotocc inode_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "inode_lifetime tests behaved as expected."
  exit 0
fi
