#!/usr/bin/env bash
#
# Regression driver for the sock_lifetime property module.
#
# One test: unit exercise of the ghost store + sock_live
...
echo "=== unit: sock_lifetime ghost + sock_live predicate ==="
run_gotocc sock_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "sock_lifetime tests behaved as expected."
  exit 0
fi
