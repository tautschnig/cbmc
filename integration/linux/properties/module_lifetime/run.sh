#!/usr/bin/env bash
#
# Regression driver for the module_lifetime property module.
#
# One test: unit exercise of the ghost store + module_live
...
echo "=== unit: module_lifetime ghost + module_live predicate ==="
run_gotocc module_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "module_lifetime tests behaved as expected."
  exit 0
fi
