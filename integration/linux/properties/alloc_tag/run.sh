#!/usr/bin/env bash
#
# Regression driver for the alloc_tag property module.
...
echo "=== unit: alloc_tag ghost + predicates ==="
run_gotocc alloc_tag.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "alloc_tag tests behaved as expected."
  exit 0
fi
