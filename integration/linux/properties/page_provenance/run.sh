#!/usr/bin/env bash
#
# Regression driver for the page_provenance property module.
#
# Three independent tests:
#
#   unit:    the store/read-back invariant holds in the reference
#            implementation.  Expect VERIFICATION SUCCESSFUL.
#
#   replace: --replace-call-with-contract on an API with a
#            page_prov_of(p) requires clause propagates the obligation
#            to call sites.  Expect VERIFICATION FAILED on the bad
#            caller's precondition.
#
#   enforce: --dfcc + --enforce-contract set_page_prov proves that
#            the reference implementation's body obeys its frame
#            condition.  Compiled with PAGE_PROV_TABLE_SIZE=3 for
#            DFCC tractability.  Expect VERIFICATION SUCCESSFUL.
#
# Exit code 0 iff all three outcomes match expectation.

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)
source "$SCRIPT_DIR/../../scan/_lib.sh"
cd -- "$SCRIPT_DIR"

fail=0
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT

echo "=== unit: store / read-back invariant ==="
run_gotocc page_provenance.c test_unit.c -o "$tmp/unit.gb"
out=$(run_cbmc "$tmp/unit.gb" --unwind 32 --unwinding-assertions 2>&1)
if echo "$out" | grep -q "^VERIFICATION SUCCESSFUL\$"; then
  echo "  [ok] unit: VERIFICATION SUCCESSFUL"
else
  echo "  [FAIL] unit: did not see VERIFICATION SUCCESSFUL" >&2
  echo "$out" | tail -20 | sed 's/^/    /' >&2
  fail=$((fail + 1))
fi

echo
echo "=== replace: --replace-call-with-contract write_to_page ==="
run_gotocc page_provenance.c test_replace.c -o "$tmp/replace.gb"
run_gi --replace-call-with-contract write_to_page \
       "$tmp/replace.gb" "$tmp/replace.trans.gb" &>/dev/null
out=$(run_cbmc "$tmp/replace.trans.gb" --unwind 32 --unwinding-assertions 2>&1)
if echo "$out" | grep -q "^VERIFICATION FAILED\$" && \
   echo "$out" | grep -qE "precondition.*write_to_page.*caller_bad.*FAILURE"; then
  echo "  [ok] replace: VERIFICATION FAILED on caller_bad's precondition"
else
  echo "  [FAIL] replace: did not see the expected FAILED precondition" >&2
  echo "$out" | grep -E "(precondition|VERIFICATION)" | sed 's/^/    /' >&2
  fail=$((fail + 1))
fi

echo
echo "=== enforce: --dfcc + --enforce-contract set_page_prov ==="
run_gotocc -DPAGE_PROV_TABLE_SIZE=3 page_provenance.c test_unit.c \
           -o "$tmp/enforce.gb"
run_gi --dfcc main --enforce-contract set_page_prov \
       "$tmp/enforce.gb" "$tmp/enforce.trans.gb" &>/dev/null
out=$(CBMC_TIMEOUT=120 run_cbmc "$tmp/enforce.trans.gb" --unwind 5 \
                                --unwinding-assertions 2>&1)
if echo "$out" | grep -q "^VERIFICATION SUCCESSFUL\$"; then
  echo "  [ok] enforce: VERIFICATION SUCCESSFUL"
else
  echo "  [FAIL] enforce: did not see VERIFICATION SUCCESSFUL" >&2
  echo "$out" | tail -25 | sed 's/^/    /' >&2
  fail=$((fail + 1))
fi

if [[ $fail -eq 0 ]]; then
  echo
  echo "All three page_provenance tests behaved as expected."
  exit 0
fi

echo >&2
echo "$fail case(s) did not match expectation." >&2
exit 1
