#!/usr/bin/env bash
#
# Regression driver for the rcu_critical_section property module.
#
# One test: unit exercise of the depth-counter ghost + predicates.
...
echo "=== unit: rcu_critical_section depth ghost + predicates ==="
run_gotocc rcu_critical_section.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "rcu_critical_section tests behaved as expected."
  exit 0
fi
