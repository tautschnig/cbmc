...
echo "=== unit: del_timer_sync_before_free armed ghost + predicate ==="
run_gotocc del_timer_sync_before_free.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "del_timer_sync_before_free tests behaved as expected."
  exit 0
fi
