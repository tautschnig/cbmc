#!/usr/bin/env bash
#
# Regression driver for the concurrent_pointer_publish property
# module.
...
echo "=== unit: concurrent_pointer_publish ghost + predicates ==="
run_gotocc concurrent_pointer_publish.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "concurrent_pointer_publish tests behaved as expected."
  exit 0
fi
