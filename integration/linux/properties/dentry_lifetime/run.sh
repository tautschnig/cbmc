#!/usr/bin/env bash
#
# Regression driver for the dentry_lifetime property module.
#
# One test: unit exercise of the ghost store + dentry_live
...
echo "=== unit: dentry_lifetime ghost + dentry_live predicate ==="
run_gotocc dentry_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "dentry_lifetime tests behaved as expected."
  exit 0
fi
