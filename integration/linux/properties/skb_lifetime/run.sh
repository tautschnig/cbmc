#!/usr/bin/env bash
#
# Regression driver for the skb_lifetime property module.
#
# One test: unit exercise of the ghost store + skb_live
...
echo "=== unit: skb_lifetime ghost + skb_live predicate ==="
run_gotocc skb_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "skb_lifetime tests behaved as expected."
  exit 0
fi
