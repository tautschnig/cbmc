#!/usr/bin/env bash
#
# Regression driver for the fput_lifetime property module.
#
# One test: unit exercise of the ghost store + fput_live
...
echo "=== unit: fput_lifetime ghost + fput_live predicate ==="
run_gotocc fput_lifetime.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "fput_lifetime tests behaved as expected."
  exit 0
fi
