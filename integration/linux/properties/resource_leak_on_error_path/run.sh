...
echo "=== unit: resource_leak_on_error_path ==="
run_gotocc resource_leak_on_error_path.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "resource_leak_on_error_path tests behaved as expected."
  exit 0
fi
