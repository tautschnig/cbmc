#!/usr/bin/env bash
#
# Regression driver for the netlink_attr_validation property module.
...
echo "=== unit: netlink_attr_validation ghost + nla_size_at_least ==="
run_gotocc netlink_attr_validation.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "netlink_attr_validation tests behaved as expected."
  exit 0
fi
