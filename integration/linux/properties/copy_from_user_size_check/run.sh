...
echo "=== unit: copy_from_user_size_check ==="
run_gotocc copy_from_user_size_check.c test_unit.c -o "$tmp/unit.gb"
...
if [[ $fail -eq 0 ]]; then
  echo
  echo "copy_from_user_size_check tests behaved as expected."
  exit 0
fi
