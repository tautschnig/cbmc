...
Copied and modified from parser-lalr-main.c from
https://github.com/rust-lang/rust

Defines function definitions for functions used in parser.y until they
are replaced with intended functionality for CBMC
...
struct node *ext_node(struct node *nd, int n, ...)
{
  ...
  print(
    "# Extending %d-ary node by %d nodes: %s = %p",
    nd->n_elems,
    c,
    nd->name,
    nd);
  ...
  nd = reinterpret_cast<node *>(realloc(nd, sz));
  nd->prev = NULL;
  nd->next = nodes;
  nodes->prev = nd;
  nodes = nd;
...
/*
Kept for reference -- delete if no longer useful
