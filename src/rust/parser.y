...
fn_decl
: fn_params ret_ty   { parser_stack($1).type() = rusttype_to_ctype(to_symbol_expr(parser_stack($2)).get_identifier());
                       $$ = $1; }
;
...
param
: pat ':' ty_sum   { //TODO_lifetimes: This will have to change to support lifetimes unless they're integrated into the rusttype_to_ctype function
                     parser_stack($1).type() = rusttype_to_ctype(to_symbol_expr(parser_stack($3)).get_identifier());
                     $$ = $1; }
;
...
ty_prim
: %prec IDENT path_generic_args_without_colons                                               { /*{o}$$ = mk_node("TyPath", 2, mk_node("global", 1, mk_atom("false")), $1);*/
                                                                                               $$ = $1; }
...
| %prec IDENT path_generic_args_without_colons '!' maybe_ident delimited_token_trees         { /*{o}$$ = mk_node("TyMacro", 3, $1, $3, $4);*/ }
| %prec IDENT MOD_SEP path_generic_args_without_colons '!' maybe_ident delimited_token_trees { /*{o}$$ = mk_node("TyMacro", 3, $2, $4, $5);*/ }
| BOX ty                                                                                     { /*{o}$$ = mk_node("TyBox", 1, $2);*/ }
| '*' maybe_mut_or_const ty                                                                  { /*{o}$$ = mk_node("TyPtr", 2, $2, $3);*/ }
| '&' ty                                                                                     { /*{o}$$ = mk_node("TyRptr", 2, mk_atom("MutImmutable"), $2);*/ }
| '&' MUT ty                                                                                 { /*{o}$$ = mk_node("TyRptr", 2, mk_atom("MutMutable"), $3);*/ }
...
macro_expr
: path_expr '!' maybe_ident parens_delimited_token_trees   { /*{o}$$ = mk_node("MacroExpr", 3, $1, $3, $4);*/ 
...
: LET pat maybe_ty_ascription maybe_init_expr ';'
  {
    ...
    if(parser_stack($4).id() != "ID_emptyinitexpr") // if given assignment expression
    {
      ...
      if(parser_stack($3).id() != "ID_emptytyascript") 
      {
        ...
        irep_idt typeName(to_symbol_expr(parser_stack($3)).get_identifier());
