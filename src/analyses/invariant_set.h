...
public:
  ...
  bool get(const exprt &expr, size_t &n);
  
  size_t add(const exprt &expr);
  
  bool is_constant(size_t n) const;
  bool is_constant(const exprt &expr) const;

  static bool is_constant_address(const exprt &expr);
  
  const irep_idt &operator[](size_t n) const
  {
    return map[n];
  }

  const exprt &get_expr(size_t n) const
  {
...
public:
  ...
  // <=
  typedef std::set<std::pair<size_t, size_t> > ineq_sett;
  ineq_sett le_set;
  ...
  typedef std::map<size_t, boundst> bounds_mapt;
  ...
  static void remove(ineq_sett &dest, size_t a)
  {
...
protected:
  ...
  void add_bounds(size_t a, const boundst &bound)
  {
    bounds_map[a].intersect_with(bound);
  }
  
  void get_bounds(size_t a, boundst &dest) const;

  // true = added s.th.
  bool make_union_bounds_map(const bounds_mapt &other);

  void modifies(size_t a);
  ...
  // queries
  bool has_eq(const std::pair<size_t, size_t> &p) const
  {
    return eq_set.same_set(p.first, p.second);
  }
  
  bool has_le(const std::pair<size_t, size_t> &p) const
  {
    return le_set.find(p)!=le_set.end();
  }
  
  bool has_ne(const std::pair<size_t, size_t> &p) const
  {
    return ne_set.find(p)!=ne_set.end();
  }
  
  tvt is_eq(std::pair<size_t, size_t> p) const;
  tvt is_le(std::pair<size_t, size_t> p) const;
  
  tvt is_lt(std::pair<size_t, size_t> p) const
  {
    return is_le(p) && !is_eq(p);
  }
  
  tvt is_ge(std::pair<size_t, size_t> p) const
  {
    std::swap(p.first, p.second);
    return is_eq(p) || is_lt(p);
  }
  
  tvt is_gt(std::pair<size_t, size_t> p) const
  {
    return !is_le(p);
  }

  tvt is_ne(std::pair<size_t, size_t> p) const
  {
    return !is_eq(p);
  }

  void add(const std::pair<size_t, size_t> &p, ineq_sett &dest);

  void add_le(const std::pair<size_t, size_t> &p)
  {
    add(p, le_set);
  }
  
  void add_ne(const std::pair<size_t, size_t> &p)
  {
    add(p, ne_set);
  }

  void add_eq(const std::pair<size_t, size_t> &eq);
