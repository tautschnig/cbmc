...
bvt float_utilst::div(const bvt &src1, const bvt &src2)
{
  ...
  size_t div_width=unpacked1.fraction.size()*2+1;
  ...
  const bvt &fraction)
{
  ...
  size_t extra_bits=fraction.size()-dest_bits;
...
void float_utilst::round_fraction(unbiased_floatt &result)
{
  ...
  if(result.fraction.size()<fraction_size)
  {
    ...
    size_t padding=fraction_size-result.fraction.size();
  ...
  {
    size_t extra_bits=result.fraction.size()-fraction_size;
    assert(extra_bits>=1);
  ...
  literalt &sticky)
{
  ...
  for(size_t stage=0; stage<dist.size(); stage++)
  {
