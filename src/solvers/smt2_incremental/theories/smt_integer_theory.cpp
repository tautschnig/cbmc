...
void smt_integer_theoryt::negt::validate(const smt_termt &operand)
{
  INVARIANT(
    ...
    "Integer theory negation may only be applied to integer sorted terms.");
}
...
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory addition may only be applied to integer sorted terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory subtraction may only be applied to integer sorted terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory multiplication may only be applied to integer sorted "
    "terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory division may only be applied to integer sorted terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory modulo may only be applied to integer sorted terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory less than may only be applied to integer sorted terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory less than or equal may only be applied to integer sorted "
    "terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory greater than may only be applied to integer sorted terms.");
}
  ...
  const smt_termt &rhs)
{
  INVARIANT(
    lhs.get_sort().cast<smt_int_sortt>() &&
      rhs.get_sort().cast<smt_int_sortt>(),
    "Integer theory greater than or equal may only be applied to integer "
    "sorted terms.");
}
