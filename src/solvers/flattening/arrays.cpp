...
arrayst::wegt::node_indext arrayst::collect_arrays(const exprt &a)
{
  ...
  if(a.id()==ID_with)
  {
    ...
    add_weg_edge(a_index, expr_old_index, with_expr);
  }
  ...
  else if(a.id()==ID_if)
  {
    ...
    std::cout << "LIT " << cond.get() << " == " << format(if_expr.cond())
              << '\n';
#endif
    add_weg_edge(a_index, expr_true_index, literal_exprt{cond});
    add_weg_edge(a_index, expr_false_index, literal_exprt{!cond});
  }
