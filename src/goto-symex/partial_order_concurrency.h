...
public:
  ...
  virtual void add_program_order(adj_matricest &po);
  virtual void add_read_from(adj_matricest &rf);
  virtual void add_write_serialisation(adj_matricest &ws);
...
protected:
  const memory_model_baset &memory_model;
  ...
  typedef std::map<evtt const*,
          std::list<std::pair<evtt const*, exprt> > > edge_to_guardt;
