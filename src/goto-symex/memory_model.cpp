    ...
    adj_matricest &ws) const
{
      ...
      ++w_it)
  {
    ...
    if(mr.empty() || w_evt2.guard.is_true())
    {
      assert(w_evt2.guard.is_true());

      // value' equals value
      equal_exprt eq(write_symbol_primed(w_evt2), w_evt2.value);
    ...
    else
    {
      ...
      equal_exprt prec_eq(write_symbol_primed(w_evt2),
          if_exprt(w_evt2.guard.as_expr(),
            w_evt2.value,
            write_symbol_primed(w_evt1)));
      poc.add_constraint(prec_eq, guardt(), w_evt2.source, "ws-preceding");
    }
