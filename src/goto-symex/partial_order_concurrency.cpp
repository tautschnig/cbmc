...
partial_order_concurrencyt::partial_order_concurrencyt(
    const memory_model_baset &_memory_model,
    ...
    const std::string &prefix) const
{
  ...
  std::string name=prefix+"$"+evt.address.as_string()+"$";
  name+=evt.value.id()==ID_symbol ?
    to_symbol_expr(evt.value).get_identifier().as_string() :
    "initval";
    ...
    const std::string &po_name)
{
  ...
  target.add_constraint(guard, expr, source);
    ...
    symbol_exprt &dest)
{
  ...
  dest=add_clock_constraint(n1_sym, n2_sym, n2.source, po_name);
}
    ...
    const exprt &cond)
{
  ...
  edge_to_guard[check][&n1].push_back(std::make_pair(&n2, cond));
}
      ...
      ++it)
  {
        ...
        ++e_it)
    {
      ...
      if(init_val.find(e.address)==init_val.end())
      {
        ...
        exprt val=sym.value;
        if(val.is_nil())
          val=typecast_exprt(false_exprt(), sym.type);
    ...
    adj_matricest &ws)
{
  for(numbered_per_thread_evtst::const_iterator it=per_thread_evt_no.begin();
      ...
      ++it)
    if(it->begin()!=it->end())
      memory_model.add_write_serialisation_internal(*this, *it, ws);
...
void partial_order_concurrencyt::acyclic()
{
  for(unsigned i=0; i<AC_N_AXIOMS; ++i)
  {
    ...
    a.operands().swap(op);
    target.add_constraint(guardt(), a, symex_targett::sourcet());
  }
...
Function: partial_order_concurrencyt::build_concrete_event_structure
...
void partial_order_concurrencyt::build_concrete_event_structure(
    const prop_convt &prop_conv,
    concrete_event_structuret &ces) const
{
  // only for debug output
  if(message.get_verbosity()<9)
    return;

  ces.initialize(per_thread_evt_no.size());
  std::set<irep_idt> initialised;
  typedef std::map<evtt const*,
          concrete_event_structuret::concrete_eventt *>
            abstract_to_concretet;
  abstract_to_concretet abstract_to_concrete;
  ...
  {
    unsigned offset=0;
    numbered_evtst::const_iterator e_it=it->begin();
    for( ; e_it!=it->end() &&
        (*e_it)->source.thread_nr!=it-per_thread_evt_no.begin();
        ++e_it)
    {
      const evtt &e=**e_it;
      exprt guard=prop_conv.get(e.guard.as_expr());
      simplify(guard, ns);
      if(guard.is_true())
        ++offset;
    }
    assert(offset>=initialised.size());
    offset-=initialised.size();

    concrete_event_structuret::concrete_eventst &ce=ces.add_processor(offset);

    for( ; e_it!=it->end(); ++e_it)
    {
      const evtt &e=**e_it;
      if(e.source.thread_nr!=it-per_thread_evt_no.begin())
        continue;
      ...
      mp_integer clock_val=0;
      if(e.direction==evtt::D_WRITE || e.direction==evtt::D_READ)
      {
        value=prop_conv.get(e.value);
        simplify(value, ns);
        if(e.direction==evtt::D_READ &&
            memory_model.steps_per_event(*this, evtt::D_READ)==2)
          to_integer(prop_conv.get(clock(AC_GHB, e, S_R_REQ)), clock_val);
        else
          to_integer(prop_conv.get(clock(AC_GHB, e, S_PROP(3))), clock_val);
        if(it==per_thread_evt_no.begin() && e.guard.is_true() &&
            e.direction==evtt::D_WRITE &&
            initialised.insert(e.address).second)
        {
          ces.add_init_event(
              concrete_event_structuret::concrete_eventt(
                0, e, value, clock_val));
      ...
      ce.push_back(
          concrete_event_structuret::concrete_eventt(
            offset++, e, value, clock_val));
      abstract_to_concrete.insert(std::make_pair(&e, &(ce.back())));
    }
  }

  for(unsigned i=AC_GHB; i<AC_GHB+1; ++i)
  {
    for(edge_to_guardt::const_iterator it=edge_to_guard[i].begin();
        it!=edge_to_guard[i].end();
        ++it)
    {
      abstract_to_concretet::iterator a_entry=
        abstract_to_concrete.find(it->first);
      if(a_entry==abstract_to_concrete.end())
        continue;

      for(std::list<std::pair<evtt const*, exprt> >::const_iterator
          it2=it->second.begin();
          it2!=it->second.end();
          ++it2)
      {
        abstract_to_concretet::const_iterator b_entry=
          abstract_to_concrete.find(it2->first);
        if(b_entry==abstract_to_concrete.end())
          continue;

        exprt cond=prop_conv.get(it2->second);
        simplify(cond, ns);
        if(cond.is_true())
          a_entry->second->happens_before.insert(b_entry->second);
        else if(it->first->direction==evtt::D_WRITE &&
            it2->first->direction==evtt::D_WRITE &&
            it->first->address==it2->first->address)
          b_entry->second->happens_before.insert(a_entry->second);
      }
    }
  }
}
