...
exprt python_convertert::convert_bin_op(const jsont &expr)
{
    ...
    op == "Add")
  {
    ...
    if(use_smt_string_backend)
    {
      ...
      // string view is reachable) and tie it to the inputs with a SINGLE
      // str.++ equality constraint (str(res) == str.++(str(left),str(right)))
      // via cprover_string_smt_concat_eq_func. This is far cheaper for CVC5's
      // string solver than byte-level array constraints, and queries on the
      // result read the same str(res) term, so they are precise.
      ...
      static unsigned smt_concat_ctr = 0;
      const unsigned uid = smt_concat_ctr++;
      ...
      symbol_exprt backing = mk("__smt_concat_data_", at);
      symbol_exprt res = mk("__smt_concat_", python_string_type());
      const exprt left_len = member_exprt{left, "length", signedbv_typet{64}};
      const exprt right_len = member_exprt{right, "length", signedbv_typet{64}};
      const exprt content = address_of_exprt(index_exprt(
        backing, from_integer(0, signedbv_typet{64}), unsignedbv_typet{8}));
      pending_checks.push_back(code_frontend_assignt{
        res,
        struct_exprt(
          {plus_exprt{left_len, right_len}, content}, python_string_type())});
      // assume str(res) == str.++(str(left), str(right))
      const typet bt = c_bool_typet{8};
      const irep_idt fn{ID_cprover_string_smt_concat_eq_func};
      if(symbol_table.lookup(fn) == nullptr)
      {
        std::vector<typet> ats{
          res.type(), to_struct(left).type(), to_struct(right).type()};
        symbolt fs{
      ...
      function_application_exprt app{
        ...
        {res, to_struct(left), to_struct(right)}};
      app.type() = bt;
      symbol_exprt rc = mk("__smt_concat_rc_", bt);
