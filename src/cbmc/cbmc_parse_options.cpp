...
void cbmc_parse_optionst::get_command_line_options(optionst &options)
{
  ...
  if(cmdline.isset("python-max-string-length"))
    options.set_option(
      ...
      cmdline.get_value("python-max-string-length"));
  ...
  if(cmdline.isset("python-raising-ops-check"))
    options.set_option("python-raising-ops-check", true);
  ...
  // --python-strict: an opt-in preset that enables the static-strictness family
  // (mypy-style). It does NOT change the default verification semantics (which
  // remain runtime-soundness oriented); it is purely a convenience that turns on
  // the declaration-enforcement + Any-erasure checks below. Runtime-exception
  // soundness (--python-raising-ops-check) is a separate axis and is
  ...
  if(cmdline.isset("python-check-any-arg-attrs") || python_strict)
  ...
  if(cmdline.isset("python-check-iter-none") || python_strict)
...
void cbmc_parse_optionst::help()
{
  ...
  std::cout << help_formatter(
    ...
    " switch\n"
    " {y--python-check-annotations} \t report values/returns/arguments that"
    " do not match declared annotations\n"
    " {y--python-missing-return-check} \t report reachable implicit"
    ...
    " Any-typed arguments not supported by any known class\n"
    " {y--python-check-iter-none} \t report iteration over a possibly-None"
    " value\n"
    " {y--python-raising-ops-check} \t model possibly-raising operations"
    " (int()/float() of non-constant strings, os.* file ops, ...) as"
    " may-raise instead of silently succeeding\n"
    " {y--python-no-exception-checks} \t do not report uncaught-exception"
    " properties\n"
    " {y--python-no-body-check} \t report calls to functions without a"
    " modelled body\n"
    " {y--python-strict-warnings} \t treat front-end conversion warnings as"
    " errors\n"
    " {y--python-lazy-stubs} \t suppress missing-method checks for stub-like"
    " classes\n"
    " {y--python-use-stdlib-source} \t prefer the real CPython stdlib source"
    " over the bundled models where available\n"
    " {y--python-ref-mutables} \t (experimental) reference semantics for"
