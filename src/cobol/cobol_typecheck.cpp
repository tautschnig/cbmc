...
/// ASCII (ISO-8859-1) → EBCDIC code page 037 translation. The front-end holds
/// character data as ASCII bytes, but z/OS uses EBCDIC, whose collating
/// sequence differs (notably: lowercase < uppercase < digits, the opposite of
/// ASCII's digits < uppercase < lowercase). Alphanumeric *ordering*
/// comparisons map each byte through this table so the result matches the
/// native (EBCDIC) collating sequence (IBM LR "Comparison of two alphanumeric
/// operands"; the native collating sequence; PROGRAM COLLATING SEQUENCE
/// defaults to NATIVE = EBCDIC). Equality is unaffected (byte equality is
/// collating-independent), so storage and the other codecs stay ASCII.
static const unsigned char ascii_to_ebcdic_cp037[256] = {
  0x00, 0x01, 0x02, 0x03, 0x37, 0x2d, 0x2e, 0x2f, 0x16, 0x05, 0x25, 0x0b, 0x0c,
  0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x3c, 0x3d, 0x32, 0x26, 0x18, 0x19,
  0x3f, 0x27, 0x1c, 0x1d, 0x1e, 0x1f, 0x40, 0x5a, 0x7f, 0x7b, 0x5b, 0x6c, 0x50,
  0x7d, 0x4d, 0x5d, 0x5c, 0x4e, 0x6b, 0x60, 0x4b, 0x61, 0xf0, 0xf1, 0xf2, 0xf3,
  0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9, 0x7a, 0x5e, 0x4c, 0x7e, 0x6e, 0x6f, 0x7c,
  ...
  ,
...
std::vector<stmtt> cobol_typecheckt::parse_divide()
{
  ...
  const auto do_remainder = [&](const valuet &dividend, const valuet &divisor)
  {
    if(!eat_word("REMAINDER"))
      return;
    // remainder = dividend - (integer-quotient * divisor) (IBM LR "DIVIDE
    // statement", REMAINDER phrase): the remainder uses the *truncated*
    // (integer) quotient, not the fractional quotient developed for GIVING.
    // After align the operands share a scale, so dividend/divisor is the
    // integer quotient (scale 0).
    const exprt int_q = div_exprt{dividend.expr, divisor.expr};
    const valuet rem{
      minus_exprt{dividend.expr, mult_exprt{int_q, divisor.expr}},
      dividend.scale};
    reft r = parse_ref();
    const bool rounded = eat_word("ROUNDED");
