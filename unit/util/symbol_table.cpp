...
TEST_CASE(
  ...
  "[core][utils][symbol_tablet]")
{
  // Like the builder, the journalling wrapper relies on the inherited
  // symbol_table_baset implementation; its clear() must reset the
  // inherited hint cache along with the wrapped table's.
  symbol_tablet symbol_table;
  journalling_symbol_tablet journalling =
    journalling_symbol_tablet::wrap(symbol_table);

  const std::string prefix = "tmp";

  const std::size_t s0 = journalling.next_unused_suffix(prefix);
  symbolt symbol;
  symbol.name = prefix + std::to_string(s0);
  journalling.insert(symbol);
  const std::size_t s1 = journalling.next_unused_suffix(prefix);
  REQUIRE(s0 < s1);

  journalling.clear();
  REQUIRE(journalling.next_unused_suffix(prefix) == 0);
}
