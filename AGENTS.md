...
- [doc/python-frontend-instance-reference-semantics-plan.md](doc/python-frontend-instance-reference-semantics-plan.md) - Reference-semantics-for-instances design & phased plan: the whole-group fix for the instance-identity cluster (local-alias `b=a`, composition field store, return-flow, context-manager mutation, concrete-class-param PUN are ALL the same root — instances are by-value at locals/fields/returns while params/`self` are already by-reference pointers). Pointer-to-struct representation, soundness (no over-aliasing), the by-value-vs-by-reference BMC perf tension (the ref_mutables cliff is nested-container `==`, not single-level instances), and the 3-phase validation-gated plan (return → local-alias → field/composition) with a landed pinning suite (`instance-*` CORE guards + KNOWNBUG acceptance criteria).
...
- [doc/python-frontend-fat-closure-plan.md](doc/python-frontend-fat-closure-plan.md) - Fat-closure design & phased plan: why per-site capture binding is unsound for anonymous channels (two demonstrated false proofs), the `python_value` CLOSURE variant + per-instance heap capture record, the arity calling-convention, boxing/dispatch sites, and the validation-gated phases that close capture-through-param / append / attribute / compose.
...
- `--python-check-annotations` — emit annotation-mismatch properties when value/return/argument types don't match declared annotations. Includes class-vs-class via MRO, reassignment after AnnAssign, `Union[...]` member checking, and container boundaries (a value incompatible with a list's concrete element type via `append`/`insert` incl. a call arg like `xs.append(src())`, or stored into a concrete-valued `dict[K,V]` via `d[k]=v`). Sound when it runs (earlier CBMC-core blockers resolved); opt-in because it enforces *static* annotations and so reports the irreducible class of real mismatches Python runs anyway (e.g. `n: int = "x"` never used as int).
- `--python-strict` — opt-in convenience preset that enables the static-strictness family (mypy-style) in one switch: `--python-check-annotations`, `--python-missing-return-check`, `--python-required-kwarg-checks`, `--python-check-typeddict-fields`, `--python-check-any-arg-attrs`, `--python-check-iter-none`. Does NOT change default semantics and does NOT imply `--python-raising-ops-check` (a separate runtime-exception-soundness axis).
