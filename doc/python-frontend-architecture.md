...
| dict params | String-keyed (key-matching) dict params now pass **by reference** (mutations propagate). A dict whose key type can't match the parameter (e.g. bare `dict`=`dict[str,value]` given int keys) still falls back to **by value** — mutation dropped | **latent unsound** for the non-key-matching residual only | [§5](python-frontend-plans.md#dict-byref) |
