...
| `none_constants` | symbol id | (set membership) | a provable `= None` binding, at the `note_mutable_extraction` chokepoint; cleared on any rebind (`invalidate_reassigned_symbol`). Lets `orderable_category_of` categorise a None-valued SYMBOL as None so `x = None; x < 0` raises TypeError (the pv-NONE lives in the ASSIGN, not the symbol value) |
...
| `--python-strict` | off | Opt-in convenience **preset** enabling the static-strictness family (mypy-style) in one switch: `--python-check-annotations`, `--python-missing-return-check`, `--python-required-kwarg-checks`, `--python-check-typeddict-fields`, `--python-check-any-arg-attrs`, `--python-check-iter-none`,
`--python-check-missing-methods`. Additive (no default-semantics change); does **not** imply `--python-raising-ops-check` (a separate runtime-exception-soundness axis) |
...
| `--python-check-missing-methods` | off | Report a call to a method not declared on the receiver's class (MRO-aware; no `__getattr__`/dynamic attrs) EVEN when an enclosing handler would catch the AttributeError. Default mode stays PLR §8.4-faithful (a caught AttributeError is real control flow -- `try/except` probing is a Python idiom); this is the mypy-style STATIC diagnostic. Part of `--python-strict` |
...
+ the --python-check-missing-methods strict-family flag). The **2026-07-22/23
ESBMC-suite refresh campaign** (see that note) then re-ran the upstream
regression suite (grown 2832 → 4079 tests, many adversarial) and drained the
danger-direction DIFFs (tests expecting a failure where we proved SUCCESSFUL)
from 66 to a small, precisely-characterised residual by closing ~11 more
CPython-confirmed false-proof classes — the PLR fold-shape rule, the
for-`continue` truncation, tuple-key iteration, conditional mutable aliasing,
tuple-target subscript swap, int-receiver AttributeError, the str/float value
folds, the pv-tuple-unpack vacuity, None-symbol and None-via-pv ordering, and
threading unlock-unheld — and by INSTITUTIONALISING the recurring
"dropped-statement / raw-key-array" anti-patterns as a static gate lint
(`scripts/lint_python_frontend.py`), which itself found two more live vacuity
channels. Two standing soundness-
...
> **Continuation (2026-07-22 → 2026-07-23) — the ESBMC-suite refresh
> campaign.** The upstream ESBMC regression suite was re-fetched and had grown
> 2832 → 4079 runnable tests (many adversarial, converted from ESBMC bug
> reports). Re-running it (`scripts/cbmc_on_esbmc_python.py`) drove the standing
> to **3563 PASS / 392 DIFF / 107 FAIL / 5 TOERR / 0 CRASH** and drained the
> DANGER-direction DIFFs (a test expects a failure but we prove SUCCESSFUL —
> the false-proof-suspect bucket) from 66 to a small residual. A methodological
> rule was established: the sweep runs at `--unwind 10`, so a danger candidate
> is re-checked at `--unwind 32` AND against CPython before it counts as a
> false proof (this alone reclassified ~1/3 as loop-truncation artifacts or
> ESBMC static-analysis-of-unreachable-code / esbmc-harness imports where we
> are PLR-correct). Closed, CPython-confirmed:
> - **PLR fold-shape soundness** (`6cef6e1ae8`): a conversion-time fold of a
>   builtin/method may only fire when it models EVERY argument
>   (`fold_covers_call_shape`) — `sum(xs, start)`, `sort(reverse=)`,
>   `min/max/sorted(key=)`, `list.index(x, start)`, `str.startswith(p, pos)`,
>   `set.union(a, b)` all folded the argument-less semantics; cheap cases are
>   now MODELLED (sum start, variadic set ops, keyed constant min/max, sorted
>   key=len). Two statement-dropping vacuities found en route (min/max nil
>   tail, `dict.fromkeys` empty-expr crash).
> - **for-`continue` truncation** (`389a0b3011`): all four `convert_for`
>   variants emitted the induction increment as a body-TAIL statement under a
>   `while`, so `continue` skipped it → an infinite goto loop silently
>   truncated into VACUOUS proofs. Converted to `code_fort` (increment as the
>   iter expression). Same commit: `for u,v in d` tuple-key unpack; conditional
>   mutable aliasing `m = l or [...]` / `(m := l)` (recorded as extraction
>   aliases → havoc); tuple-target subscript swap `a[i],a[j]=a[j],a[i]`.
> - **int-receiver AttributeError** (`29d0cf35b7`): `.foo`/`.foo()` on a
>   provable-scalar int raises (read + call paths).
> - **str/float value folds** (`86453feb36`): a shared `py_float_repr`
>   (shortest round-trip + CPython's fixed/scientific threshold, validated
>   bit-for-bit over 830 values) replaced a wrong 6-digit ostream fold across
>   `str()`/`.format()`/f-strings; `str.center` odd-padding (extra goes LEFT);
>   bounded negative-step slice (stop exclusive; full-reverse only for a
>   bound-less `[::-1]`).
> - **pv-tuple-unpack vacuity + `__file__`** (`83cee12182`): unpacking a pv
>   tuple matched no concrete branch and DROPPED the targets + assert (a
>   constant tuple-return literal is now unpacked precisely, else a sound nondet
>   floor); `__file__` is a str, not None.
> - **None ordering** (`f9d9fa0e6a`, `ca247b69cf`): a None-valued SYMBOL
>   (`none_constants`) and a python_value operand whose runtime tag is NONE now
>   raise TypeError on `<`/`>`/… instead of reading the None sentinel and
>   comparing numerically. Catchable, FP-safe.
> - **threading.Lock.release** on an unheld lock raises RuntimeError
>   (`05458f24a3`).
> - **The soundness lint** (`799eb11042`, `scripts/lint_python_frontend.py`,
>   wired into the standing gate): institutionalises the two invariant-read
>   false-proof anti-patterns (raw string into a dict KEYS array w/o
>   `coerce_element`; `return nil_exprt{}` from a `try_*` engaged-optional
>   helper → dropped statement). It immediately found and fixed two more live
>   vacuity channels (`try_builtin_call` min/max operand, `try_method_call`
>   receiver convert-failure).
> - **Unbounded-ints hardening** (`1a6a6d247d`, `a72ed6b568`): PLR §6.7 integer
>   mod/floordiv were value-wrong (nondet mod, Euclidean floordiv — all four
>   sign quadrants pinned), Int→FP typecast added to smt2 (`to_fp` of
>   `to_real`), non-constant Int→BV via `int2bv`, class INT fields → int-id
>   handles, and the structural-metadata invariant (i64 lengths vs integer
>   values) enforced at ~14 sites. Unbounded+native corpus 11 → 1 TOERR.
> Genuine danger residual (each mapped to a named representation/provenance
> pass, NOT a point fix): the `missing-return_*` cluster (None erased to the
> int SENTINEL at the argument→int-PARAMETER boundary — the
> None-through-typed-int-slot pass); `range` explicit-iterator StopIteration
> (`iter(list)` returns the list identity with no consumption cursor);
> `Decimal("0")` truthiness (the `Decimal(...)` construction fold does not fire
> for the `from decimal import Decimal` shape); `github_3001` (missing method
> on a pv-CLASS UNION receiver — provenance-refined obligation); `cmath` polar
> signed-zero (atan2 float-model). The 107 sweep FAILs are tool-output
> alignment (ESBMC exact error strings / property-count output) or
> static-analysis-of-unreachable-code, where CBMC is PLR-correct — no action.
...
| Nested mutable-element aliasing | anonymous nested-mutable list/dict elements stored by value; the replication / self-append / new-container channels are **guarded** (report `python-model-bound`). **Direct** nested mutation works (`c[i].append(...)` for lists, and int-keyed dict values via lvalue value slots). **Extraction-then-mutate** (`r=c[i]; r.append(...)` / `v=a[k]; v.append(...)`) is layered (2026-07-19/20): for DICT values with an int-keyed slot or a constant STRING key, AND (2026-07-22) for LIST elements at a CONSTANT index, the mutation **writes through** to the source slot — fully precise (`extracted_slot_alias`, §0; `extraction-write-through` / `extraction-key-write-through` / `extraction-list-write-through` CORE), and this now covers all FOUR in-place channels (mutator method, aug-assign, subscript store, `del`); everything else keeps the 2026-06-24 sound guard — the extracted variable is recorded (`extracted_container_alias`) and a subsequent in-place mutation havocs the source container AND every sibling alias of it (+ drops the const-fold maps). **For LISTS, the principled whole-group fix is now implemented behind opt-in `--python-ref-mutables`** (2026-06-24): nested list elements are heap-allocated per instance and aliased by pointer (`make_python_value(LIST, allocate_boxed_leaf(rebuild_list_as_pv(e)))`), so extraction / multi-instance / reorder / membership / nested value reads / 3-deep composition are **precise**; under the flag the havoc guard is skipped for wrapped references. | sound for the CONTAINER-ELEMENT aliasing channels (guarded). **The distinct
...
| **None through a typed-int PARAMETER** (missing-return cluster) | a function fall-through / `return None` into an `int`-annotated return slot emits the int SENTINEL (`-4611686018427387904`), not a pv-NONE; when that value flows on as a call ARGUMENT bound to an `int`-typed PARAMETER, `param < 0` is a numeric compare on the sentinel — no TypeError (`missing-return_*`, CPython raises `TypeError: '<' not supported ... NoneType`). The **pv-operand** face IS closed (`ca247b69cf`, `pv-none-ordering-fail`): when None reaches the comparison AS a python_value its NONE tag now raises. The PARAMETER-boundary face needs None-preservation across the argument→int-slot coercion — the same **None-through-typed-slot representation pass** as the return slot. Sound direction is unaffected (this is a false PROOF residual, opt-in `--python-missing-return-check` flags the definition site) | **residual** (default proves SUCCESSFUL on the reachable None path); flag-covered at the definition site | [plan §0](python-frontend-plan.md#false-proofs) |
| `range` explicit-iterator StopIteration | `it = iter(range(2)); next(it); next(it); next(it)` should raise StopIteration; `iter(list)`/`iter(range)` returns the underlying identity with NO consumption cursor, so `next` re-reads. The for-loop and Name-alias generator cursors ARE precise (see the 2026-07-01 generator note); this is the EXPLICIT-iterator extension of that machinery (bounded, cross-cutting) | **residual** (recorded, needs iterator cursors) | [plan §1](python-frontend-plan.md#generators) |
| `Decimal("0")` truthiness | `bool(Decimal("0"))` is False; the `Decimal(...)` construction literal-parsing fold does not fire for the `from decimal import Decimal` binding shape (the name binds the CLASS object, `__class_tag 11`), so `__bool__` reads a nondet `_int` — a Decimal-model pass is needed (a probe regressed truthy Decimals and was reverted) | **residual** (recorded, needs the Decimal-model pass) | [decimal plan](python-frontend-decimal-plan.md) |
| Missing method on a pv-CLASS UNION receiver | `f = create(0)` where `create() -> Any` returns `Foo()` on one branch, `Bar()` on another; `f.bar()` is an AttributeError only on the `Foo` arm (`github_3001`). The identity-refined dispatch admits the arm whose owner HAS the method and does not obligate the arms whose owner lacks it — needs the provenance-refined missing-method obligation (obligate every arm whose class lacks the member). The concrete-receiver and single-class-provenance cases ARE covered | **residual** (recorded, provenance-refined obligation) | [plan §1](python-frontend-plan.md#generators) |
...
| Modules | `cmath` polar/rect signed-zero + NaN (`atan2(+0.0, -0.0)` is π, not 0.0 — a float-model detail in the `cmath` stub, `cmath_polar_rect_semantics_*`); fuller `os`/`time`/`datetime`/`json`/`dataclasses`/`collections` not modelled (nondet) | [plan §6](python-frontend-plan.md#modules) |
