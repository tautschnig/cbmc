...
fixes; default-on measured ~1.25% FP and declined); and **fixed-tuple slicing**
(`t[1:]` returns a correct tuple — a real correctness bug). The **8 remaining**
oracle items are: DEEP representation work — **c2** (dict structural-mutation
through a call) and **c4 ≡ laurel-006** (`del`+`__getattr__` retype, 2 of the 8,
one root); the **annotation-laundering family caught under
`--python-check-annotations` with default-on declined** — **004**/**007**
(arg/element) and **ty-010** (return annotation, reclassified 2026-06-29: it is
*not* a variable-length-tuple gap but the `-> str` annotation being trusted); and
other settled — **binop-mirror-evalorder** (pinned KNOWNBUG) and **d1**
