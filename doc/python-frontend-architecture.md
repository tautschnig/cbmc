...
fixes; default-on measured ~1.25% FP and declined). The **8 remaining** oracle
items are either DEEP representation work (ty-010 variable-length-tuple
Any-erasure; c2 dict structural-mutation-through-call; c4/laurel-006
`del`+`__getattr__` retype) or settled decisions (`004`/`007` flag-gated;
`binop-mirror-evalorder` pinned KNOWNBUG; `d1` numeric-tower documented
out-of-subset) — see the rows below.
...
| Variable-length-tuple slice → str method (ty-010) | `tuple[int, ...]` is modelled as `python_value` (Any), so `t[1:]` is Any and a `-> str` return annotation is trusted → `f(...).upper()` is accepted though the runtime tuple has no `.upper()` | **OPEN, deep**: needs a real variable-length-tuple model (symbolic length + element type). The concrete-receiver str-method variant is CLOSED above | [plan §0](python-frontend-plan.md#false-proofs) |
...
| `del obj.attr` + `__getattr__` fallback (c4/laurel-006) | after `del self.x`, access falls to `__getattr__` returning a different type; the concretely-typed field cannot hold it, so the cross-type `TypeError`/wrong-value is missed | **OPEN, deep**: needs per-instance field-presence tracking or `python_value` fields (fixed-struct-model limitation; `del` currently havocs the slot to nondet — sound but doesn't model the `__getattr__` retype) | [plan §10](python-frontend-plan.md#descriptors) |
...
| Tuples | fixed-tuple **slicing** `t[1:]` is mis-modelled (the slice result is not a proper tuple — `len`/index of it are wrong); `tuple.count`/`index` exact-value asserts not always proven (precision; pre-existing, surfaced during the str-method/unpack work) | [plan §9](python-frontend-plan.md#precision) |
...
| Descriptors / dynamic attrs | method shadowing, **data descriptors** (`__set__` + stateful `__get__`), **`@property` getters AND setters** (a setter is a data descriptor — `obj.p = v` runs the setter; accessors stored under distinct symbols so a setter no longer clobbers the getter, 2026-06-29), and forward-referenced field access via a generic param are now modelled; **remaining**: truly-dynamic attribute *names* (`setattr(o, computed, v)`) need a runtime instance-`__dict__`, and `del obj.x` + `__getattr__`-retype fallback (c4/laurel-006, see soundness table) | [plan §10](python-frontend-plan.md#descriptors) |
