...
| **Narrowing-invalidation cluster** (mostly OPEN false proofs) | a value's runtime type changes via an effect cbmc does not model, then it is used at the stale type → CPython `TypeError`, cbmc verifies. Distinct roots (**not one fix**). **CLOSED:** `__setattr__`/`__getattribute__` (2026-06-25, over-approx to nondet `python_value`); **enum `.value` after a member retag** (2026-06-28: heterogeneous-enum value type is `python_value` + `.value` resolves on enum-typed variables AND fields, so the retagged value routes through the operand/tag obligations — `enum-value-after-mutation` is now CORE). **OPEN, and characterized 2026-06-28 as the SAME reference-semantics root:** context-manager `__enter__`/`__exit__` field mutation AND composition aliasing are both the *concrete-class-typed slot copies the instance* problem (a `t: SomeClass` param/field is value-copied, so a mutation through it is invisible to the original; the Any-typed path preserves identity by-address). Also OPEN: inheritance+union virtual dispatch, same-expression **eval-order × union-retag** | partly **UNSOUND** (the reference-semantics false proofs remain), confined to advanced/dynamic features; pinned (`shared-object-aliasing-knownbug`, `instance-aliasing-knownbug`, `context-manager-enter-mutation-knownbug`, `union-use-after-mutation-typeerror-knownbug`). The context-manager + composition cases unify with the **Class-instance identity** row above (a dedicated reference-semantics-for-instances effort closes them together) | [plan §0](python-frontend-plan.md#false-proofs) |
...
| Class-instance identity / aliasing | a class instance is value-semantics (struct copy) at a top-level `b = a` assignment, so a mutation through one alias is invisible to the other (`b = a; b.x = 99; a.x` proves the stale 42). Distinct from the list/dict case, which DOES alias (the `alias_targets` pointer mechanism). **P2 investigation (2026-06-26):** extending `alias_targets` to instances does NOT work as a one-line change — instance attribute read/write does not route through the alias pointer the way list subscript does (the per-instance-identity / attribute-field-reference problem; a spike confirmed no effect). The **viable sound fix is a havoc-on-mutation guard** (analogous to the extraction-then-mutate guard: record `b = a` instance aliases, havoc the other on a mutation through either), deferred to a focused effort. | **CLOSED 2026-06-28** (reference-semantics-for-instances, Phases 1-3): instances are by-reference at returns, local aliases, and fields -- so return-flow, local alias `b=a`, AND composition/field store all preserve identity. `instance-return-aliasing`, `instance-aliasing`, `instance-field-aliasing`, `shared-object-aliasing`, `context-manager-enter-mutation` are all CORE. Soundness: a FRESH construction stays owned/by-value (distinct, no over-aliasing). The deep-composition `==` perf cliff did NOT materialise, so this is default-on (unlike container ref-semantics) | [instance ref-semantics plan](python-frontend-instance-reference-semantics-plan.md) + [plan §0](python-frontend-plan.md#false-proofs) |
