...
| List element, **annotated** — `append`/literal-init (`xs: list[int] = []; xs.append(u)` / `[u]`) | the element is kept `python_value` (tag preserved) → use-site obligation fires | **sound** (2026-07-10 audit; `slot-pun-list-element-append-sound`(+`-nofp`) CORE) |
| List element, **annotated** — subscript-store / `insert` (`xs: list[int]; xs[0]=u` / `xs.insert(0,u)`) | the explicit `int` element type **puns** the `python_value`/mismatched value (coerced via `coerce_element`/unwrap, losing the tag) | **annotation-laundering** (`ORACLE-INTRINSIC`; `slot-pun-list-element-knownbug`) — would need slot-widening |
...
subscript-store/`insert` path — and `slot-pun-attr-field-knownbug`). **Partial
progress (2026-07-10 audit):** the `list[int]` `append`/literal-init paths now
keep the element `python_value` (tag preserved), so those paths are sound
(`slot-pun-list-element-append-sound`(+`-nofp`) CORE); the subscript-store /
`insert` paths and the annotated attribute `field: int` still pun. They are the
same mechanism as the `ORACLE-INTRINSIC` annotation-laundering residuals in the
CURRENT STATE header (`007` list-element, `ty-010` field/return) — a real false
proof, classified intrinsic because it depends on a (wrong) static annotation
Python never enforces.
...
| Variable-length-tuple slice → str method (ty-010) | `def f(t) -> str: return t[1:]` then `f(...).upper()`: the **`-> str` return annotation is trusted**, so `f(...)` is typed `str` and `.upper()` is accepted even though the body returns a tuple/list | **Annotation-laundering family (same as 004/007), flag-gated.** Caught under `--python-check-annotations` — the *return*-annotation-mismatch check fires on `return t[1:]` (a sequence vs declared `str`). DEFAULT trusts the annotation (declined for the same measured ~1.25% benign-FP reason as 004/007); pinned `vartuple-slice-strmethod-knownbug` (which slices a DIRECT literal `(1,2,3)[1:]` to exercise the laundering cleanly). **Caveat (2026-07-10 audit):** a `tuple[...]`-ANNOTATED **parameter** (fixed OR variadic) is currently treated as *not subscriptable* — a separate sound false ALARM (see the "tuple-annotated parameter subscript" row below) — so the earlier `def f(t: tuple[int,...]): return t[1:]` form was masked by a spurious TypeError before reaching the `.upper()` laundering. A fixed-tuple slice on a *literal receiver* is modelled exactly; the parameter-subscript gap is the separate precision item | [plan §7](python-frontend-plan.md#check-annotations) |
...
| Tuples | fixed-tuple **slicing** `t[1:]` is now modelled exactly for constant bounds (step 1, negatives, `[::-1]`; `tuple-slice` CORE) — **was** mis-modelled (wrong `len`/index). Remaining: SYMBOLIC-bound or general-step tuple slices fall through (over-approx); a `tuple[int, ...]` variable-length sequence model (vs Any) is unbuilt — a precision item. A `tuple[...]`-ANNOTATED **parameter** (fixed `tuple[int,int]` OR variadic `tuple[int,...]`) is currently treated as *not subscriptable* — indexing or slicing it (`t[i]`, `t[1:]`) raises a spurious TypeError (sound false ALARM; a direct-literal receiver `(1,2,3)[1:]` is fine) — found by the 2026-07-10 doc audit. `tuple.count`/`index` exact-value asserts not always proven | [plan §9](python-frontend-plan.md#precision) |
