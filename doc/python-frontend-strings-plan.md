...
| `split` (list-valued result) | both | hard everywhere (variable-count result); native residual |
...
| **native `== ""` (empty-string literal)** | native | **ROBUSTNESS BUG found 2026-06-22** — comparing a string to the empty literal `""` (specifically `nondet_string(0) == ""`) emits malformed SMT2 (`VERIFICATION ERROR`, "line 2: unexpected token" / "non-Boolean value for B0"); `!= ""`, `"" == ""`, `"x" == ""`, and `nondet_string(N>0) == ""` are all fine. Sound (errors, never a false proof). **Partial root cause:** cbmc declares the native-string intrinsics (`cprover_string_*_func`) as SMT *functions* and then emits `(get-value (cprover_string_length_func))` — a malformed get-value of an un-applied function — which corrupts the solver's value-response stream. Fix shape: in `smt2_conv` `find_symbols`, exclude `ID_mathematical_function`-typed symbols from the get-value set (declare them, don't get-value them). That is a **shared-core** change (affects all SMT2 solving) needing the full C-SMT2 regression to validate, and it did not by itself clear the interactive-cvc5 "line 2" facet — so it is deferred to a dedicated, properly-validated shared-core change rather than landed unvalidated. |
| **native case-transform + `len()` perf** | native | **PERF cliff found 2026-06-22** — `upper`/`lower`/`casefold`/`swapcase`/`capitalize`/`title` build an `O(PYTHON_MAX_STRING_LENGTH=64)` `str.++` concat chain; a `len()` query over that on an *unconstrained* symbolic string times out in both cvc5 and z3 (constrained content is fast: `s.upper()=="ABC"` under `assume(s=="abc")` is 0 s). Corpus-invisible. Fix shape (Python-frontend-only): emit a fresh `smt_string` `r` with `assume(str.len(r)==str.len(s))` + bounded per-position char-map equalities, instead of the deep concat — so `len(r)` is constrained directly and the solver needn't fold a 64-deep concat's length. **Entanglement (2026-06-22):** this fix needs a `str.len` constraint on the fresh result, which routes through `cprover_string_length_func` — the same un-applied SMT function behind the `== ""` get-value bug above — so the `smt2_conv` get-value fix is a prerequisite for landing the case-transform re-encoding safely. Both are corpus-invisible and best done together as one dedicated, full-C-SMT2-validated native-robustness change. |
