...
| I8 | **PARTIAL** — a non-constant reference-modification length is carried as `dyn_size`; a group/alphanumeric MOVE sender and alphanumeric comparison honour it exactly (CORE `refmod-dynamic-length`, `refmod-dynamic-compare`). STRING/UNSTRING and refmod receivers still use the static upper bound (sound; `cobol:refmod-range` flags unsafe cases) | "Reference modification" | — | precision-and-gaps §6 |
