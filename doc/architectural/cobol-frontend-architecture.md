...
| I8 | **PARTIAL** — a non-constant reference-modification length is carried as `dyn_size`; the MOVE sender, alphanumeric comparison, and STRING/UNSTRING honour it exactly (CORE `refmod-dynamic-{length,compare,string,unstring}`). Only a refmod *receiver* (`MOVE X TO Y(s:n)`) still uses the static upper bound (sound; `cobol:refmod-range` flags unsafe cases) | "Reference modification" | — | precision-and-gaps §6 |
