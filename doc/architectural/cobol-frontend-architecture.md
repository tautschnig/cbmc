...
| G2 | **RESOLVED** — EBCDIC (CP037) character model (see S4); signed zoned trailing/leading overpunch now modelled; SIGN SEPARATE remains | "USAGE DISPLAY" | — | numeric-encoding §5 |
...
| I4 | **PARTIAL** — MOVE to a numeric-edited PICTURE applies Z/`*` suppression, `.`, `,`, B/0//, a fixed leading sign (`+`/`-`), fixed leading `$`, and trailing `CR`/`DB` (CORE `move-edited`, `edited-sign`); only *floating* sign/currency (`++++`, `----`, `$$$$`) still falls back to nondet | "MOVE statement"; "PICTURE clause" editing | — | numeric-encoding |
...
| I7 | **RESOLVED** — the subscript-range bound for an `OCCURS … DEPENDING ON` table is the run-time value of the DEPENDING item (per-dimension bound is now an expr), not the static maximum (CORE `occurs-depending`); storage is still sized at the maximum | "OCCURS clause" (format 2) | — | precision-and-gaps §2 |
