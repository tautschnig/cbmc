...
| I8 | **PARTIAL** — a non-constant reference-modification length is carried as a `dyn_size` on the operand; a group/alphanumeric MOVE sender copies exactly that many bytes (CORE `refmod-dynamic-length`). Comparison/STRING/UNSTRING and refmod receivers still use the static upper bound (sound; `cobol:refmod-range` flags unsafe cases) | "Reference modification" | — | precision-and-gaps §6 |
