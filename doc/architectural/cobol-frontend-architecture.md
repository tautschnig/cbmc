...
| I4 | **RESOLVED** — MOVE to a numeric-edited PICTURE applies suppression (Z/`*`), insertion (`.` `,` B 0 /), fixed sign/currency (`+`/`-`/`$`, `CR`/`DB`), and floating sign/currency (`++++`/`----`/`$$$$`) (CORE `move-edited`, `edited-sign`, `edited-float`) | "MOVE statement"; "PICTURE clause" editing | — | numeric-encoding |
